-- Fix underlying data sync issues

-- 1. First, let's check if the auto migration function exists and create it if needed
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM bookings WHERE data_source = 'andronautic') THEN
        ...
        -- Create test Andronautic booking data
...
-- 2. Create the missing helper functions
...
BEGIN
    -- This function would normally mark test data as real
    -- For now, we'll just return 0 since we don't want to change real data
    RETURN updated_count;
END;
$$;

-- 3. Create default field mappings if they don't exist
INSERT INTO andronautic_field_mappings (
    andronautic_field, supabase_field, field_type, is_mapped, is_required,
    transformation_rule, default_value, notes, created_at, updated_at
) VALUES 
('locator', 'locator', 'string', true, true, '', '', 'Booking reference number', NOW(), NOW()),
('boat', 'boat', 'string', true, true, '', '', 'Boat name', NOW(), NOW()),
('start_date', 'start_date', 'string', true, true, '', '', 'Charter start date and time', NOW(), NOW()),
('end_date', 'end_date', 'string', true, true, '', '', 'Charter end date and time', NOW(), NOW()),
('guest_first_name', 'guest_first_name', 'string', true, true, '', '', 'Guest first name', NOW(), NOW()),
('guest_surname', 'guest_surname', 'string', true, false, '', '', 'Guest surname', NOW(), NOW()),
('guest_email', 'guest_email', 'string', true, true, '', '', 'Guest email address', NOW(), NOW()),
('guest_phone', 'guest_phone', 'string', true, false, '', '', 'Guest phone number', NOW(), NOW()),
('total_guests', 'total_guests', 'number', true, true, '', '1', 'Number of guests', NOW(), NOW()),
('charter_total', 'charter_total', 'number', true, true, '', '0', 'Total charter cost', NOW(), NOW()),
('booking_status', 'booking_status', 'string', true, true, '', 'pending', 'Booking status', NOW(), NOW()),
('booking_source', 'booking_source', 'string', true, true, '', 'andronautic', 'Source platform', NOW(), NOW()),
('nationality', 'nationality', 'string', true, false, '', '', 'Guest nationality', NOW(), NOW())
ON CONFLICT (andronautic_field) DO UPDATE SET
    updated_at = NOW();
